    ...
    onChange() {
      ...
      for (const file of inputs) {
        ...
        if (type === 'image' && this.limitBytes && file.size >= this.limitBytes) {
          ...
          if (mb >= 1) {
            this.showError(this.$t('uploader.errors.fileSizeLimit'), this.$tc('meta.mb', { count: mb }));
          } else {
            this.showError(this.$t('uploader.errors.fileSizeLimit'), this.$tc('meta.kb', { count: kb }));
          }
          // eslint-disable-next-line no-continue
          continue;
        }
